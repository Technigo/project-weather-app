...
const time = date.toLocaleTimeString().substring(0, 5); //date.getHours() + ':' + date.getMinutes();
console.log(time);
...
btnSearchCity.addEventListener('click', () => {
  ...
  fetch(url)
    ...
    .then(function (data) {
      ...
      <h6>Sundet: ${new Date(data.city.sunset * 1000).toLocaleTimeString().substring(0, 4) } </h6>
       `      
