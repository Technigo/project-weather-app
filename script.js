...
const createCityInput = (parentElement, inputValue = displayedCityName) => {
  ...
  // Append the input to the parentElement
...
/**
 * Helper to format times based on local time in the city
 */
...
/**
 * Helper to set weather today content
 */
const setWeatherTodayContent = (
  temp,
  description,
  mainTitle,
  imgSrc,
  ...
  // Define the sunrise and sunset icons
  const sunriseIcon = `
  ...
  weatherTodayContainer.innerHTML = `
    ...
    <div class="weather-today__meta">
      ...
      <p class="badge" id="todays-sunrise" aria-hidden="true">${sunriseIcon} ${sunrise}</p>
      <p class="sr-only">Time of sunset today is ${sunset}</p>
      <p class="badge" id="todays-sunset" aria-hidden="true">${sunsetIcon} ${sunset}</p>
    </div>
  `;
};

/**
 * Simplified version of the getCurrentWeather function
 */
const getCurrentWeather = async (mockType = null) => {
  try {
    const weatherRightNow = await fetchWeather(
      getWeatherUrl(apiQueryCity),
      mockType
    );
    if (!weatherRightNow) {
      weatherTodayContainer.innerHTML = `
    ...
    const { name, weather, main, sys, timezone } = weatherRightNow;
    displayedCityName = name;
    ...
    // Update H1 input
    const parts = mainTitle.split("{city}");
