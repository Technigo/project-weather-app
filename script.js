...
const API_WEATHER = `https://api.openweathermap.org/data/2.5/weather?q=Stockholm,Sweden&units=metric&APPID=${API_KEY}`;
const API_FORECAST = `https://api.openweathermap.org/data/2.5/forecast?q=Stockholm,Sweden&units=metric&APPID=${API_KEY}`;
...
fetch(API_WEATHER) // first fetch to get todays weather
  .then(response => {
...
fetch(API_FORECAST) // second fetch to get 5 day forecast
  .then(response => {
    return response.json();
  })
  .then(forecastArray => {
