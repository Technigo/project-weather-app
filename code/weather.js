...
const currentWeatherUrl =
  "https://api.openweathermap.org/data/2.5/weather?q=Toronto,Canada&units=metric&APPID=c2889b12ee617ea787319a19a98a5906";
const weatherForecastUrl =
  "https://api.openweathermap.org/data/2.5/forecast?q=Toronto,Canada&units=metric&APPID=c2889b12ee617ea787319a19a98a5906";
...
const currentWeatherToday = (json) => {
  ...
  const sunriseTorontoTime = sunriseToronto.toLocaleTimeString("en-CA", {
    timeStyle: "short",
  });
  ...
  const sunsetTorontoTime = sunsetToronto.toLocaleTimeString("en-CA", {
    timeStyle: "short",
  });
...
const changeBackground = (json) => {
  if (
    json.main.temp.toFixed(0.5) <= 10 ||
    json.main.feels_like.toFixed(0.5) <= 10
  ) {
    body.classList.add("cold");
  } else if (
    json.main.temp.toFixed(0.5) >= 22 ||
    json.main.feels_like.toFixed(0.5) >= 2
  ) {
...
const updateMinMaxTemps = (data) => {
  ...
  for (const date in minMaxTemps) {
    ...
    forecast.innerHTML += `<div class="column">${minMaxTemps[
      date
    ].minTemp.toFixed(0.5)} °C|${minMaxTemps[date].maxTemp.toFixed(
      0.5
    )} °C </div>`;
