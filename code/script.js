...
function createFiveDaysInnerHTML(
    days,
    forecastTemp,
    forecastWind,
    weatherIcon
...
function changeHeaderInnerHTML(data, timeOfSunrise, timeOfSunset, weatherIcon) {
    today.innerHTML = `
        ...
        <h5 class="windpar">Windspeed: ${Math.round(data.wind.speed * 10) / 10
        } m/s</h5>
        <p class="sunrisepar"><img src=${sunriseIcon.src} alt=${sunriseIcon.alt
        } class= "sunrise"/> ${timeOfSunrise}</p>
        <p class="sunsetpar"><img src=${sunsetIcon.src} alt=${sunsetIcon.alt
