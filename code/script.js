...
fetch('https://api.openweathermap.org/data/2.5/forecast?q=Stockholm,Sweden&units=metric&APPID=8802f8b4b2d622931613aace44be57ae')
	...
	.then((json) => {
		...
		//json.list[0].weather[0].main = "Rain"
		...
		// This will filter the json response using includes(timestamp) and display a div with the forecast info
		let filteredForecast = json.list.filter(item => item.dt_txt.includes('12:00'));
		filteredForecast.forEach(item => { 
			forecastWrapper.innerHTML += `
				<div class="forecast-row">
					<div class="for-day-temp">${new Date(item.dt * 1000).toLocaleDateString("en-US", { weekday: "short", })}</div>
