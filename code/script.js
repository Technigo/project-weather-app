...
let currentTown = "Örebro";

// Store API URL's
const weatherApiUrl = `https://api.openweathermap.org/data/2.5/weather?q=${currentTown},Sweden&units=metric&APPID=${API_KEY}`;
const forecastApiUrl = `https://api.openweathermap.org/data/2.5/forecast?q=${currentTown},Sweden&units=metric&APPID=${API_KEY}`;
...
fetch(weatherApiUrl)
  ...
  .then((json) => {
    ...
    if (currentTime < sunriseValue || currentDate > sunsetValue) {
